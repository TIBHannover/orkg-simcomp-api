...
class ContributionSimilarityService(OrkgSimCompApiService):
    ...
    def __init__(self, orkg_backend: OrkgBackendWrapperService, es_service: ElasticsearchService):
        ...
        self.index_name = os.getenv('ORKG_SIMCOMP_API_ES_CONTRIBUTIONS_INDEX', 'contributions')
    ...
    def index(self, contribution_id):
        ...
        if not self.es_service.exists(index=self.index_name):
            self.logger.warning('Index "{}" does not exist. Initializing...'.format(self.index_name))
            self.init_index()
