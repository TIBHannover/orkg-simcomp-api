...
class ThingService(OrkgSimCompApiService):
    ...
    def add_thing(self, thing_type: ThingType, thing_key: str, data: Dict[str, Any]):
    ...
    def get_thing(self, thing_type: ThingType, thing_key: str):
        thing = self.crud_service.get_row_by(
            entity=Thing,
            columns_values={
                'thing_type': thing_type,
    ...
    def export_thing(self, thing_type: ThingType, thing_key: str, format: ExportFormat):
        # TODO: add like_ui logic. Perhaps in **kwargs schema !?
        thing = self.get_thing(thing_type, thing_key)['thing']

        try:
            return {
                ThingType.COMPARISON: ComparisonExporter
            }[thing_type].export(thing.data, format)
        except KeyError:
            raise OrkgSimCompApiError(
