...
from app.models.thing import ThingAddRequest, ThingGetResponse, ThingType, ExportFormat
...
def adds_thing(
    ...
    return ResponseWrapper.wrap_json(service.add_thing(request.thing_type, request.thing_key, request.data))


@router.get('/', response_model=ThingGetResponse, status_code=http.HTTPStatus.OK)
@log(__name__)
def gets_thing(
...
@router.get('/export', response_model=Response, status_code=http.HTTPStatus.OK)
@log(__name__)
def exports_thing(
        thing_type: ThingType,
        thing_key: str,
    ...
    if format == ExportFormat.CSV:
        return StreamingResponse(
            iter([export]),
            media_type='text/csv',
            headers={'Content-Disposition': 'attachment; filename={}_{}.csv'.format(thing_type.lower(), thing_key)}
        )
