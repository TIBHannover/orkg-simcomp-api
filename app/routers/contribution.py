...
def compares_contributions(
    ...
    comparison = service.compare(contribution_ids=contributions, comparison_type=type, format=format)

    if format == ExportFormat.CSV:
        return StreamingResponse(
            iter([comparison]),
            media_type='text/csv',
            headers={'Content-Disposition': 'attachment; filename={}.csv'.format('_'.join(contributions))}
        )
